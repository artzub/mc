...
do_panelize_cd (struct WPanel *panel)
{
    ...
    if (strcmp (panelized_panel.root, panel->cwd) == 0)
        panelized_same = TRUE;
    ...
    for (i = 0; i < panelized_panel.count; i++)
    {
        ...
        {
            list->list[i].fname = g_build_filename (panelized_panel.root,
                                                    panelized_panel.list.list[i].fname, NULL);
