...
sftpfs_readdir (void *data)
{
    ...
    if (rc <= 0)
    {
        vfs_print_message (_("sftpfs: Listing done."));
        return NULL;
    }
...
sftpfs_open_socket (struct vfs_s_super *super)
{
    ...
    for (curr_res = res; curr_res != NULL; curr_res = curr_res->ai_next)
    {
        ...
        vfs_print_message (_("sftpfs: making connection to %s"), super->path_element->host);
...
sftpfs_do_connect (struct vfs_class *me, struct vfs_s_super *super)
{
    ...
    if (super->path_element->password == NULL)
    {
        ...
        if ((SUP->auth_pw & 4) != 0)
            p = g_strconcat (_("sftpfs: Enter passphrase for"), " ", super->path_element->user,
                             " ", NULL);
        else
            p = g_strconcat (_("sftpfs: Enter password for"), " ", super->path_element->user,
                             " ", NULL);
        ...
        if (super->path_element->password == NULL)
        {
            vfs_print_message (_("sftpfs: Password is empty."));
            goto err_conn;
        }
    }
